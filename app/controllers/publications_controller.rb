class PublicationsController < ApplicationController
  ...
  def update
    ...
    respond_to do |format|
      if params[:publication][:researchers]
        researcher = Researcher.find(params[:publication][:researchers].to_i)
        @publication.researchers << researcher
        @publication.save
        format.json { render json: researcher }
      elsif params[:publication][:research_areas]
        research_area = ResearchArea.find(params[:publication][:research_areas].to_i)
        @publication.research_areas << research_area
        @publication.save
